...
import com.ximalaya.omp.distributor.api.model.DistributeItemType;
...
@Slf4j
@Order(2)
public class PlaceOrderParamsCheckAspectV2 implements ParamsCheck{

    /**
     * 分销业务参数校验
     */
    @Override
    @Around("@annotation(com.ximalaya.omp.openapi.payment.aspect.annotation.ParamsCheck.PlaceOrderParamsCheckV2)")
    public Object paramValidate(ProceedingJoinPoint pjp) throws Throwable{
        ...
        Integer distributeItemType = (Integer) args[distributeItemTypeIndex] == null ? -1 : (Integer) args[distributeItemTypeIndex];
        ...
        boolean needRiskControlParamsExist = payCommonQueryParam.isNeedRiskControlParamsExist();
        String deviceAgent = payCommonQueryParam.getDeviceAgent();
        String deviceIP = payCommonQueryParam.getDeviceIP();

        //---------------------------------------------------------------

        //根据开关 needRiskControlParamsExist 校验是否需要风控参数必须出现  riskControlParamsCheck
        ...
        int clientOsType = payCommonQueryParam.getClientOsType();
        //        distributionParamsCheck(distributeItemType, accessAccountType, clientOsType, payContent, payPrice);
